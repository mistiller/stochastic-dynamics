...
class SyntheticDataset:
    ...
    def generate(self) -> Dataset:
        ...
        raw_allocations = (
            np.random.rand(self.T) + 1e-6
        )  # Add small epsilon to avoid zero if all rand are 0
        ...
        input_series = np.clip(
            input_series, 1e-9, None
        )  # Ensure inputs are positive for power functions

        # Generate cost and benefit series based on the input
        # Cost = base_cost * input^d_t_value + noise
        cost = self.base_cost * (input_series**self.d_t_value) + np.random.normal(
            scale=self.cost_noise, size=self.T
        )

        # Benefit = base_benefit * input^scale_benefit + noise
        benefit = self.base_benefit * (
            input_series**self.scale_benefit
        ...
        return Dataset(t, input_series, cost, benefit)
