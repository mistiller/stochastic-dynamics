...
class ParameterEstimator:
    ...
    def _build_model(self):
        ...
        with self.model:
            ...
            base_cost = pm.HalfNormal(
                "base_cost", sigma=self.priors["base_cost"]["sigma"]
            )
            base_benefit = pm.Normal(
                "base_benefit",
                mu=self.priors["base_benefit"]["mu"],
                sigma=self.priors["base_benefit"]["sigma"],
            )
            scale_benefit = pm.HalfNormal(
                "scale_benefit", sigma=self.priors["scale_benefit"]["sigma"]
            )
            ...
            ell = pm.Gamma(
                "ell",
                alpha=self.priors["gp_ell"]["alpha"],
                beta=self.priors["gp_ell"]["beta"],
            )
            mean_d = pm.Normal(
                "mean_d",
                mu=self.priors["gp_mean"]["mu"],
                sigma=self.priors["gp_mean"]["sigma"],
            )
            ...
            pm.Normal(
                "benefit_obs", mu=benefit_model, sigma=0.1, observed=self.data.benefit
            )

    def get_parameters(
        self, draws=1000, tune=1000, chains=4
        ...
        return ParameterEstimationResult(
            base_cost={
                "mu": float(post.base_cost.mean()),
                "sigma": float(post.base_cost.std()),
            },
            base_benefit={
                "mu": float(post.base_benefit.mean()),
                "sigma": float(post.base_benefit.std()),
            },
            scale_benefit={
                "mu": float(post.scale_benefit.mean()),
                "sigma": float(post.scale_benefit.std()),
            },
            gp_eta_prior={"mu": float(post.eta.mean()), "sigma": float(post.eta.std())},
            gp_ell_prior={"mu": float(post.ell.mean()), "sigma": float(post.ell.std())},
            gp_mean_prior={
                "mu": float(post.mean_d.mean()),
                "sigma": float(post.mean_d.std()),
            },
            trace=trace,
        )
