...
class PriorDefinition(BaseModel):
    ...
    def create_pymc_distribution(self, name: str) -> pm.Distribution:
        ...
        try:
            ...
            if self.dist in ["Beta", "Gamma"]:
                if "mu" in self.params and "sigma" in self.params:
                    ...
                    if mu is None or sigma is None:
                        raise ValueError(
                            f"{self.dist} prior for {name} requires either 'alpha'/'beta' or 'mu'/'sigma'. Got {self.params}"
                        )
                    ...
                    if self.dist == "Beta":
                        ...
                        if variance >= max_variance:
                            logger.warning(
                                f"Variance {variance:.4f} for Beta prior '{name}' is too large for mean {mu:.4f}. Clamping variance."
                            )
                        ...
                        if sum_ab <= 0:
                            logger.warning(
                                f"Calculated alpha+beta <= 0 for Beta prior '{name}'. Using default Beta(1,1)."
                            )
                            alpha = 1.0
                            beta = 1.0
                        else:
                        ...
                        logger.info(
                            f"Converted mu={mu:.3f}, sigma={sigma:.3f} to Beta(alpha={params['alpha']:.3f}, beta={params['beta']:.3f}) for '{name}'."
                        )
                        ...
                        if variance <= 0:
                            logger.warning(
                                f"Variance {variance:.4f} for Gamma prior '{name}' is non-positive. Using default Gamma(1,1)."
                            )
                            alpha = 1.0
                            beta = 1.0
                        else:
            ...
            logger.exception(
                f"Error creating PyMC distribution '{self.dist}' for '{name}' with params {self.params}: {e}"
            )
            raise
